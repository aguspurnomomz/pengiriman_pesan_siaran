...
export function PatientsPage() {
  ...
  // ============ FUNGSI SANITASI FLEKSIBEL ============
  
  // 1. Sanitasi nomor WhatsApp (menangani +62, 62, 08, dan 8)
  const sanitizePhoneNumberFlexible = (phone: string): string => {
    if (!phone) return "";
    
    // Hapus semua karakter non-digit
    let cleaned = phone.replace(/\D/g, "");
    
    // Jika diawali dengan '62' (internasional) -> konversi ke '0'
    if (cleaned.startsWith("62") && cleaned.length >= 11) {
      cleaned = "0" + cleaned.slice(2);
    }
    // Jika diawali dengan '8' dan panjang 10-12 digit (kehilangan 0 di depan)
    else if (cleaned.startsWith("8") && cleaned.length >= 10 && cleaned.length <= 12) {
      cleaned = "0" + cleaned;
    }
    // Jika diawali dengan '0' dan panjang >= 10 -> sudah benar
  ...
  // 2. Sanitasi tanggal lahir (menangani berbagai format: DD/MM/YYYY, DD-MM-YYYY, DD/MM/YY, dll)
  const sanitizeDateOfBirthFlexible = (dateStr: string): string => {
    if (!dateStr) return "";
    
    let cleaned = dateStr.trim();
    
    // Konversi semua separator (/ atau . atau spasi) menjadi -
    ...
    if (parts.length === 3) {
      ...
      // Jika tahun hanya 2 digit (misal: 21 untuk 2021)
      if (year.length === 2) {
        const currentYear = new Date().getFullYear();
        const currentCentury = Math.floor(currentYear / 100) * 100;
        year = (parseInt(year) + currentCentury).toString();
        
        // Jika tahun masih terlalu jauh ke depan (misal 2025 > tahun sekarang + 10 tahun)
  ...
  // 5. Validasi tanggal lahir dengan format fleksibel
  const validateDateOfBirthFlexible = (dateStr: string): boolean => {
    if (!dateStr) return false;
    
    // Coba parse dengan berbagai format
    const testFormats = (str: string): Date | null => {
      // Format YYYY-MM-DD
    ...
    const date = testFormats(dateStr);
    if (!date) return false;
    
    // Cek apakah tanggal valid (tidak NaN) dan tidak lebih dari hari ini
    return !isNaN(date.getTime()) && date < new Date();
  };

  // 6. Format nomor WhatsApp ke format Indonesia (08xx)
  const formatWhatsAppNumber = (number: string) => {
    ...
    // Jika diawali dengan 8 (kehilangan 0 di depan)
    else if (cleaned.startsWith("8") && cleaned.length >= 10 && cleaned.length <= 12) {
      return "0" + cleaned;
    }
  ...
  // Proses Import Excel (Bulk Upsert) dengan Sanitasi Fleksibel
  const handleImportExcel = async (event: React.ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      ...
      for (let i = 0; i < rows.length; i++) {
        ...
        let dateOfBirthRaw = row["Tanggal Lahir"]?.trim();
        ...
        // === SANITASI NOMOR WHATSAPP FLEKSIBEL ===
        if (!phoneNumberRaw) {
          logs.push({
            row: rowNumber,
            name: name,
        ...
        // Sanitasi nomor WhatsApp (menangani +62, 62, 08, 8)
        const sanitizedPhone = sanitizePhoneNumberFlexible(phoneNumberRaw);
        const formattedPhone = formatWhatsAppNumber(sanitizedPhone);

        if (!validateWhatsApp(sanitizedPhone)) {
          logs.push({
            row: rowNumber,
            name: name,
            phone_number: phoneNumberRaw,
            status: "failed",
            message: `Format nomor WhatsApp tidak valid. Input: "${phoneNumberRaw}" → Hasil sanitasi: "${sanitizedPhone}"`
          });
          failedCount++;
          continue;
        }

        // === SANITASI TANGGAL LAHIR FLEKSIBEL ===
        if (!dateOfBirthRaw) {
          logs.push({
            row: rowNumber,
            name: name,
        ...
        // Sanitasi tanggal lahir (konversi berbagai format ke YYYY-MM-DD)
        const sanitizedDate = sanitizeDateOfBirthFlexible(dateOfBirthRaw);

        if (!validateDateOfBirthFlexible(sanitizedDate)) {
          logs.push({
            row: rowNumber,
            name: name,
            phone_number: formattedPhone,
            status: "failed",
            message: `Tanggal lahir tidak valid. Input: "${dateOfBirthRaw}" → Hasil sanitasi: "${sanitizedDate}"`
          });
          failedCount++;
          continue;
        }

        // Gunakan sanitizedDate untuk proses selanjutnya
        const finalDateOfBirth = sanitizedDate;
        ...
        if (existingPatient) {
          ...
          if (error) {
            ...
            logs.push({
              ...
              phone_number: formattedPhone,
              status: "updated",
              message: `Data berhasil diperbarui (Menimpa data lama) - Tanggal lahir: ${finalDateOfBirth}`
            });
            updatedCount++;
          }
        } else {
          ...
          if (error) {
            ...
            logs.push({
              ...
              phone_number: formattedPhone,
              status: "success",
              message: `Data berhasil ditambahkan - Tanggal lahir: ${finalDateOfBirth}`
            });
            successCount++;
          }
        }
  ...
  const handleAddPatient = async () => {
    ...
    try {
      ...
      if (existingPatient) {
        setErrorMessage(`Pasien dengan nama "${formData.name}" dan nomor HP "${formattedPhone}" sudah terdaftar!`);
        setShowErrorToast(true);
        setTimeout(() => setShowErrorToast(false), 3000);
        setIsSaving(false);
        return;
  ...
  return (
    <div className="space-y-6">
      <div>
        <h2 className="text-2xl font-semibold tracking-tight">Data Pasien Penerima Pesan</h2>
          ...
          <div className="flex items-center justify-between flex-wrap gap-2">
            ...
            <div className="flex gap-2">
              ...
              <Dialog open={isImportDialogOpen} onOpenChange={setIsImportDialogOpen}>
                ...
                <DialogContent>
                  <DialogHeader>
                    <DialogTitle>Import Data Pasien dari Excel</DialogTitle>
                    <DialogDescription>
                  ...
                  <div className="space-y-4 py-4">
                    ...
                    <div className="space-y-2">
                      ...
                      <div className="text-xs text-blue-600 space-y-1 mt-2">
                        <p>💡 <span className="font-semibold">Tips Format Data:</span></p>
                        <p>• Nomor WhatsApp: <code className="bg-blue-100 px-1 rounded">081234567890</code>, <code className="bg-blue-100 px-1 rounded">+6281234567890</code>, atau <code className="bg-blue-100 px-1 rounded">6281234567890</code> → otomatis dikonversi ke <code className="bg-blue-100 px-1 rounded">08xxx</code></p>
                        <p>• Tanggal Lahir: <code className="bg-blue-100 px-1 rounded">1990-05-15</code>, <code className="bg-blue-100 px-1 rounded">15/05/1990</code>, <code className="bg-blue-100 px-1 rounded">15-05-90</code>, atau <code className="bg-blue-100 px-1 rounded">15/05/21</code> → otomatis dikonversi ke <code className="bg-blue-100 px-1 rounded">1990-05-15</code></p>
                      </div>
                    </div>
                    {isImporting && (
                        ...
                        <div className="flex flex-col items-center gap-2 text-muted-foreground">
                          ...
                          <p className="text-sm">
                            {searchQuery
                              ...
                              : "Klik 'Tambah Pasien' untuk mulai menambahkan data"}
                          </p>
                        </div>
                      </TableCell>
                    </TableRow>
      ...
      <Dialog open={isImportResultDialogOpen} onOpenChange={setIsImportResultDialogOpen}>
        <DialogContent className="max-w-3xl max-h-[80vh] overflow-y-auto">
          <DialogHeader>
            ...
            <DialogDescription>
              Berikut adalah hasil import data pasien dari file Excel
            </DialogDescription>
          </DialogHeader>
